...
[**Figure 2.4 placeholder — Structured and irregular sparsity through a CPU cache hierarchy.** Compare two sparse matrix rows with the same number of nonzeros. In the structured case, block-adjacent values share cache lines, support prefetching, and are reused before eviction. In the irregular case, compressed-row indices lead to dependent gathers from widely separated addresses; show index and value metadata, page translations, full cache-line fills containing only one or a few useful words, a working set larger than the last-level cache, and reuse distances long enough for eviction before reuse. Include a useful-bytes-to-bytes-transferred indicator. State explicitly that structured or reordered sparsity need not thrash caches; the pathological behavior belongs to large, weakly local, dynamically indexed sparsity.]
...
**Problem: MoE routing is data-dependent.** Token counts per expert vary, and expert placement may require all-to-all communication. The bottleneck is routing balance, fine-grained network traffic, and synchronization. **Direct consequence:** overloaded experts become stragglers, underloaded experts waste capacity, network congestion limits scale, and tail step time rises even though only a subset of parameters is active.
...
Google’s first-generation TPU illustrates the economic logic rather than a universal replacement rule. It targeted a large, stable production-inference portfolio with a matrix unit, software-managed on-chip memory, and an execution model chosen in part for predictable 99th-percentile response [1.12]. The specialization was valuable because workload volume amortized the hardware and software investment. A smaller or rapidly changing workload might rationally remain on GPUs even at lower silicon efficiency because flexibility, availability, and software maturity have economic value.

## 2.14 Future Workload Stress Test: Predictive World Models
...
## 2.15 Architectural Synergy Across the Compute Continuum
...
[**Figure 2.6 placeholder — Bidirectional migration of accelerator principles across the compute continuum.** Place hyperscale/datacenter, enterprise/private infrastructure, non-mobile edge, client/mobile, and embedded/robotics along a continuum. Show shared principles—locality, explicit movement, quantization, right-sized parallelism, heterogeneous scheduling, and power gating—moving in both directions. Under each deployment, annotate the different primary objective and implementation constraint. Avoid implying that the same physical chip is reused unchanged.]

## 2.16 Engineering Takeaways

- Workload labels hide distributions of shapes, locality, state, dependency, arrivals, and deadlines; therefore, architecture decisions should use workload signatures rather than model names or peak operation counts.
- Software can remove avoidable work and expose the remaining constraint, so hardware is warranted only after a residual bound and its direct system consequence are demonstrated.
- Technical feasibility does not establish economic viability; “hardware necessity” is therefore conditional on lifecycle volume, energy, capacity, service objectives, software cost, and risk.
- Dense training combines arithmetic with capacity, input supply, synchronization, and communication, which means time and energy to a quality target—not isolated kernel throughput—determine which resource deserves acceleration.
- Prefill and decode have different parallelism, state, and bandwidth signatures, so one accelerator may remain the best flexibility compromise, or phase specialization may become economical when its transfer and operational costs are amortized.
- Recommendation, retrieval, sparsity, MoE, and graph workloads contain indirect access, metadata, imbalance, and communication; thus, reducing multiply count or adding dense throughput does not solve their critical path unless the architecture also addresses movement and scheduling.
- GPUs remain highly programmable but are most efficient with abundant parallelism, coalescible access, regular lane work, adequate occupancy, and amortized overhead, making non-GPU designs credible when workload semantics keep important phases outside that efficiency envelope at material scale.
- Future world models may change state, temporal, planning, and control behavior rather than merely increase dense computation, so the taxonomy must remain model-family-independent.
- Client and mobile inference is governed by whole-system wake-up, memory, battery, and sustained thermal behavior; therefore, the correct hardware boundary may be a power domain, memory path, or near-sensor engine rather than a larger NPU.
- Robotics and automotive systems require bounded response, isolation, and safe degraded behavior, so average throughput cannot substitute for deterministic system integration.
- Architectural principles migrate across datacenter, enterprise, mobile, and embedded systems while physical implementations diverge; accordingly, reuse should be sought in principles, parameterized IP, software abstractions, and operational knowledge—not assumed at the level of an unchanged chip.
