...
**Snapshot date: mid-2026.** This chapter and the next are the survey’s lowest-stability layer by design (Charter §16): they record how the architecture families of Chapter 6 are instantiated commercially at the time of writing. Product names, generations, and market events below were verified against public sources in July 2026 and will age; the analytical claims—which family each product instantiates, which necessity-test conditions its position depends on—are intended to outlive the specific parts. Facts resting on reputable reporting rather than vendor or peer-reviewed disclosure are marked evidence class D.
...
[**Figure 7.1 (draft produced) — The 2026 flagship memory landscape.** File: `figures/fig07-01-flagship-memory-2026.svg` (also `.png`). Capacity versus bandwidth for the mid-2026 flagship accelerators (B300-class, MI355X, MI400 announced, Ironwood, Trainium3, Maia 200), extending Figure 6.8’s axes to the current product generation; per-package comparison level; announced-but-unshipped parts marked as class B. The clustering of every design between 144–432 GB and 5–20 TB/s makes the chapter’s point: the industry has converged on attacking the same memory bound.]

## 7.2 Hyperscaler Tensor Processors

### The §2.14 verdict, replicated four times

Chapter 2 executed the necessity test on TPU v1 and found all four conditions holding at owner scale. The mid-2026 landscape shows that verdict replicated at every major cloud, each instance a Chapter 6 §6.3 tensor processor with owner-specific emphases:
...
The synthesis is sobering and clarifying: of the independent challengers Chapter 6 used as family exemplars, by mid-2026 one licensed itself into the incumbent, one survives on anchor tenancy at IPO scale, one is in acquisition talks, and one persists as a niche appliance vendor. *None failed on architecture.* Every trajectory was decided by §2.14 condition 4—software capital, volume, and distribution—exactly as the Nervana and Graphcore precedents indicated. The contested middle is resolving not by architectural victory but by ownership convergence: the surviving deployment paths for beyond-GPU datacenter silicon run through hyperscaler internal programs, incumbent platforms, or anchor-tenant contracts.
...
### Table 7.1 — Mid-2026 enterprise accelerator landscape (per-package figures unless noted)
...
Read against the survey’s thesis, the mid-2026 enterprise landscape supports four conclusions.
